...
setAuthority('admin');
LS.set('activityId', 10); // TODO 活动ID

// const bgmusic = require('@/audio/bgm.mp3');
const shareIcon = require('@/assets/share-icon.jpg');
...
if (!isWeixn()) {
  ...
  debug().then(() => {
    initWx({
      ...
      isNeedLogin: true,
      desc: '副标题',
      openid: process.env.NODE_ENV === 'development' ? '你的 openid' : undefined,
      appId: 'wx******************', // 微信公众号 appId
    });
  });
}
